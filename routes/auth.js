...
router.post('/login', async (req,res)=>{
  ...
  try {
    // here we will search in our database in the User model if there is a user with username property that carries the same value of the username sent in request or with email property carries the same value of email sent in request so user can enter username or email
    // if we find that then put this user in variable user so if the user entered a correct username or email a user from database will be in the user variable
    const user = req.body.username? await User.findOne({ username: req.body.username }) : await User.findOne({email: req.body.email});
    // if user variable is null so there is no user with the username or email sent in the request
    if (!user) {
      return res.status(401).json({ msg: "Wrong Username or Email, please try again" });
    };
