...
// DELETE
// delete cart 
// here again we used the middleware function that we made for cart authorization as the id in parameter is cart's id not user's id
...
// here we used user's id to help fetch this api in front end as if a user logged out then logged in again we lose the id of the cart so we can't show it unless saving it in the local storage but if another one logged in from the same device it won't be safe
