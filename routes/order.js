...
router.post("/create", verifyToken, async (req, res) => {
  ...
  try {
    ...
    await Cart.deleteOne({userId: req.body.userId}); // here we delete the cart of the user, the cart that carries userId property with the same value as req.body.userId sent to fill the req.body in creating an order so when user creates an order the cart is deleted as user should only have one cart so we can show it to the user by the get method in cart
